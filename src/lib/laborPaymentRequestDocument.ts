...
import {
  ...
  COMPANY_NAME, BRAND_NAVY, type DocumentGradientKey,
...
export type PrWorkerLine = {
  ...
  /** A qualifying line under the payee in the breakdown — payroll uses it to
   *  say what was withheld, since Rate and Amount alone show a gross and a
   *  net with the deduction between them unexplained. */
  subNote?: string | null
  bankAccount: string | null
  /** Which bank holds that account. An account number alone is not a payment
   *  instruction — a CBE bulk transfer cannot carry a Bank of Abyssinia
   *  account, so a run spanning banks has to be split before it reaches the
   *  bank, and the document is where that split has to be visible. */
...
export type LaborPaymentRequestInput = {
   ...
   *  A single color paints a flat band; it also tints the type-detail header. */
  accentColor?: string | null
  /** A named gradient for the letterhead, for document kinds that get their
   *  own identity in DOCUMENT_GRADIENTS rather than borrowing an expense
   *  type's flat accent. Wins over accentColor for the band; accentColor
   *  still tints the type-detail header, so set both. */
  accentGradient?: DocumentGradientKey | null
  ...
  /** What one row of a labor-shaped breakdown is. Defaults to "worker",
   *  which is right for site labor and wrong for a payroll run — the people
   *  on it are employees, and calling them workers is part of what made a
   *  payroll document read as a site payslip. */
  breakdownNoun?: string | null
...
/**
 * Splits the disbursement schedule the way the money actually has to move:
 * one block per bank. A payroll run is the case that forces it — a single
 * run routinely covers accounts at two or three banks, and each bank can only
 * be handed its own block — but it applies to any request whose payees bank
 * in different places.
 *
 * Payees with no bank recorded are collected into a final unnamed group
 * rather than dropped or lumped in with the largest bank, because "we don't
 * know where this person banks" is exactly what the person preparing the
 * transfer needs to see.
 */
export function groupPayeesByBank(payees: PrPayeeLine[]): PrBankGroup[] {
  const byBank = new Map<string, PrBankGroup>()
  for (const p of payees) {
    const key = p.bankName ?? NO_BANK
  ...
  return Array.from(byBank.values()).sort((a, b) => {
    if ((a.bank === null) !== (b.bank === null)) return a.bank === null ? 1 : -1
...
export function buildPayeeLines(workers: PrWorkerLine[], opts?: { isLabor?: boolean }): PrPayeeLine[] {
  ...
  for (const w of workers) {
    ...
    const bankName = (useVendor ? w.vendorBankName : w.bankName) ?? null
...
${renderLetterhead({
    ...
    metaLines,
    gradient: accentGradient ?? (accentColor ? { from: accentColor, to: accentColor } : 'laborPayment'),
  })}
