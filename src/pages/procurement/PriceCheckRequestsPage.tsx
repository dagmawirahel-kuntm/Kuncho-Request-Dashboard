                    ...
                    <td className="px-4 py-2 text-slate-700 dark:text-slate-200">
                      {r.stock_items ? (
                        ...
                        <>
                          ...
                          <span className="block text-[10px] text-slate-400">new item · {r.sub_categories?.item_name ?? '—'}</span>
                        </>
                      ) : (
                        <>
                          <span className="font-medium">{r.sub_categories?.item_name ?? '—'}</span>
      ...
      <div className="w-full max-w-md rounded-xl bg-white dark:bg-slate-800 shadow-xl border dark:border-slate-700 p-5 space-y-3" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <div>
            ...
            {request.stock_items?.item_name
              ?? request.item_description
              ?? request.sub_categories?.item_name
              ?? '—'}
          </p>
          </div>
