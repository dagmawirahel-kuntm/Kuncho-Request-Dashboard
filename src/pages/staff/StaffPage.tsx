...
function StaffFormModal({ record, onClose }: { record?: Staff; onClose: () => void }) {
  ...
  const [form, setForm] = useState<Partial<StaffInsert>>(
    isEdit
      ? { employee_name: record.employee_name, staff_type: record.staff_type, role: record.role, monthly_salary: record.monthly_salary ?? undefined, day_rate: record.day_rate ?? undefined, payment_frequency: record.payment_frequency, bank_account: record.bank_account, starting_date: record.starting_date, termination_date: record.termination_date, phone_number: record.phone_number, experience: record.experience }
      : {}
  )
  ...
  async function handleSave() {
    ...
    qc.invalidateQueries({ queryKey: ['staff'] }); onClose()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-lg font-semibold">{isEdit ? 'Edit Staff' : 'New Staff Member'}</h2>
    ...
    { accessorKey: 'phone_number', header: 'Phone', cell: ({ getValue }) => getValue() ?? '—' },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => handleDelete(row.original.id, row.original.employee_name)} className="rounded p-1 text-slate-400 hover:bg-red-50 hover:text-red-600" title="Delete">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-bold text-slate-800">Staff</h1><p className="text-sm text-slate-500">Employee records and HR data</p></div>
