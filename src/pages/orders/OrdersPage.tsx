...
function OrderFormModal({ record, onClose }: { record?: Order; onClose: () => void }) {
  ...
  const [form, setForm] = useState<Partial<OrderInsert>>(
    isEdit
      ? { item_service_description: record.item_service_description, order_date: record.order_date, quantity: record.quantity ?? undefined, status: record.status, notes: record.notes, vendor_recommendation: record.vendor_recommendation }
      : { status: 'pending' }
  )
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Quantity"><input type="number" className={inputCls} value={form.quantity ?? ''} onChange={e => set('quantity', e.target.value ? parseFloat(e.target.value) : null)} /></Field>
          </div>
          <Field label="Status">
