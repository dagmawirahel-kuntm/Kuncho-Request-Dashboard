...
function OrderFormModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState<Partial<OrderInsert>>({ status: 'pending' })
  ...
  async function handleSave() {
    ...
    const { error } = await supabase.from('orders').insert([form as any])
    setSaving(false)
    if (error) { setError(error.message); return }
    qc.invalidateQueries({ queryKey: ['orders'] }); onClose()
  }
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Order Date"><input type="date" className={inputCls} value={form.order_date ?? ''} onChange={e => set('order_date', e.target.value)} /></Field>
            <Field label="Quantity"><input type="number" className={inputCls} value={form.quantity ?? ''} onChange={e => set('quantity', parseFloat(e.target.value))} /></Field>
          </div>
          <Field label="Status">
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button onClick={handleSave} disabled={saving} className="rounded-md bg-slate-900 px-4 py-2 text-sm text-white hover:bg-slate-800 disabled:opacity-60">{saving ? 'Saving…' : 'Save Order'}</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800"><Plus className="h-4 w-4" /> New Order</button>
      </div>
      {isLoading ? <div className="py-12 text-center text-sm text-slate-400">Loading…</div> : <DataTable columns={columns} data={data} searchPlaceholder="Search orders…" />}
