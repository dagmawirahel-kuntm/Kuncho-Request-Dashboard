-- ── Fix duplicate expense_code values (FK-safe) ───────────────────────────────
...
-- purchase_allocation.parent_purchase_id (no cascade)
...
-- tool_units.expense_id (ON DELETE SET NULL)
UPDATE tool_units
SET expense_id = NULL
FROM _exp_map m
WHERE expense_id = m.dupe_id;

-- stock_receipts.expense_id (ON DELETE SET NULL)
...
-- order_expenses.expense_id (ON DELETE CASCADE — just delete the link rows)
