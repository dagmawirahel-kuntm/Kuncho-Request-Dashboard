-- ── Step 1: clean up FK references and delete duplicates ─────────────────────
DO $$
BEGIN
  ...
  UPDATE purchase_allocation
    SET parent_purchase_id = m.keeper_id FROM _exp_map m
    WHERE parent_purchase_id = m.dupe_id;

  UPDATE transportation_requests
    SET expense_id = m.keeper_id FROM _exp_map m
    WHERE expense_id = m.dupe_id;

  UPDATE cpo_bonds
    SET related_expense_id = m.keeper_id FROM _exp_map m
    WHERE related_expense_id = m.dupe_id;

  UPDATE tool_units    SET expense_id = NULL FROM _exp_map m WHERE expense_id = m.dupe_id;
  UPDATE stock_receipts SET expense_id = NULL FROM _exp_map m WHERE expense_id = m.dupe_id;
  UPDATE sourcing_bundles SET expense_id = NULL FROM _exp_map m WHERE expense_id = m.dupe_id;
...
-- ── Step 2: DDL must run outside the DO block ─────────────────────────────────
