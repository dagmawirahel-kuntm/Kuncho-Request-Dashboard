-- 238's rollup used array_agg(ts.gang_member_staff_ids ORDER BY ts.date DESC)[1]
-- to pick the most recent day's gang roster. That's broken two ways:
--
-- 1. gang_member_staff_ids is itself uuid[], so array_agg() over it
--    produces a 2-D array. Indexing a 2-D array with a single [1]
--    subscript does not return "row 1" -- Postgres collapses it to a
--    scalar uuid (NULL in practice), which is exactly the live error:
--    column "gang_member_staff_ids" is of type uuid[] but expression
--    is of type uuid.
-- 2. Even before that: if two days in the same rollup period have
--    gang selections of different sizes (5 people one day, 4 the
--    next), array_agg() over the raw array column throws "cannot
--    accumulate arrays of different dimensionality" -- a hard error,
--    not just a wrong value.
--
-- Fix: drop the array_agg/index trick and pull the latest non-null
-- gang_member_staff_ids with a plain correlated subquery instead.
